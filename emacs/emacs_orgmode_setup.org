...
-----
=M-x magit-status=	Displays status buffer of your git repo

In Status Buffer:

=?= =C-c C-e= =C-c C-c= magit-key-mode-popup-dispatch
...
=c=			Commiting, pop up a buffer for writing commit messages. type "C-c C-c" when finished

=k=			Discard changes/Delete (untracked) file at point
-----
The above lines are formated using ~C-u M-x align-regexp RET =\(\s-+\)[^] RET RET n~
- ~=\(\s-+\)[^]~ :: regexp: Match lines. =\s-= means whitespace in emacs lisp.
