...
Start from ~v2.9.0~, Magit sets ~INSIDE_EMACS~ internally[fn:2]. To use
pinentry in Emacs's minibuffer:
#+BEGIN_EXAMPLE
  # First, start Emacs's internal pinentry server
  M-x pinentry-start↲

  # Second, show Magit's panel
  M-x magit-status↲
#+END_EXAMPLE
